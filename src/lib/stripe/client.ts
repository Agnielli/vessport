...
// Función para calcular el total del carrito con lógica de 10 ventas
export const calculateCartTotal = (items: CartItem[]) => {
  ...
  // Calcular tarifas de diseño (€80 si no ha alcanzado 10 ventas)
  const designFees = items.reduce((sum, item) => {
    if (item.designId && item.designSalesCount < 10) {
      return sum + (80 * item.quantity) // €80 por cada camiseta con diseño no gratuito
    }
    return sum
  }, 0)
